...
export default async function () {
  ...
  try {
    ...
    try {
      ...
      logger.setResults(headingVisible ? (await resultsPage.getResultsHeadingText()) + ' (unfiltered)' : 'NOT VISIBLE')
    }
    ...
    if (vitals.fcp) { fcpMetric.add(vitals.fcp); logger.setFCP(vitals.fcp) }
    if (vitals.lcp) { lcpMetric.add(vitals.lcp); logger.setLCP(vitals.lcp) }
